...
function analyzeString($str, $pat) {
	var pat = _xregexp2.default.cache((0, _xvtype._first)($pat), "g");
	var str = (0, _xvtype._first)($str);
	var ret = [];
	var index = 0;
	_xregexp2.default.replace(str, pat, function () {
		...
		if (a.length > 0) {
			var c = a.reduce(function (pre, _, i) {
				if (_ !== undefined) {
					return pre.concat((0, _xvnode.element)("fn:group", (0, _xvtype.seq)((0, _xvnode.attribute)("nr", i + 1 + ""), (0, _xvnode.text)(_))));
				} else {
					return pre;
				}
			}, (0, _xvtype.seq)());
...
function tokenize($str, $pat) {
	...
	var str = (0, _xvtype._first)($str).valueOf();
	if (str !== "") {
...
function substring($_, $a, $b) {
	return item($_).map(function (_) {
		return _.substring((0, _xvtype._first)($a), (0, _xvtype._first)($b));
	});
}

function stringToCodepoints($str) {
	return (0, _xvtype.toSeq)((0, _xvtype._first)($str).split("")).map(function (a) {
		return a.codePointAt();
	});
}

function codepointsToString($seq) {
	return (0, _xvtype.seq)($seq.map(function (_) {
...
function matches($str, $pat) {
	var pat = _xregexp2.default.cache((0, _xvtype._first)($pat), "g");
	var str = (0, _xvtype._first)((0, _xvtype.string)($str)).valueOf();
...
function replace($str, $pat, $rep) {
	var pat = (0, _xvtype._first)($pat).valueOf();
	var rep = (0, _xvtype._first)($rep).valueOf();
	var str = (0, _xvtype._first)((0, _xvtype.string)($str)).valueOf();
	...
	if (!_cache.has(str, pat, rep)) {
		ret = _cache.set(str, pat, rep, _xregexp2.default.replace(str, _xregexp2.default.cache(pat), rc[rep], "all"));
	} else {
...
function stringLength($str) {
	var str = (0, _xvtype._first)($str);
	return (0, _xvtype.seq)(str.length);
}

function stringJoin($seq, $sep) {
	var sep = (0, _xvtype._first)($sep);
...
function concat() {
	...
	return (0, _xvtype.seq)((0, _xvtype.string)((0, _xvtype.toSeq)(a)).flatten(true).join(""));
}