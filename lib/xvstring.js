...
function analyzeString($str, $pat) {
	...
	var pat = (0, _xregexp2.default)((0, _xvtype._first)($pat), "g");
	var ret = [];
	var index = 0;
	if (str) {
		str = str.toString();
...
function tokenize($str, $pat) {
	...
	if (str) {
		str.toString().split(pat).forEach(function (s) {
...
function substring($_, $a, $b) {
	return (0, _xvtype._first)((0, _xvtype.string)($_)).substring((0, _xvtype._first)($a), (0, _xvtype._first)($b));
}

function stringToCodepoints($str) {
	return (0, _xvtype.toSeq)((0, _xvtype._first)((0, _xvtype.string)($str)).split("")).map(function (a) {
		return a.codePointAt();
	});
}

function codepointsToString($seq) {
	return (0, _xvtype.seq)($seq.reduce(function (acc, _) {
